...
pub struct TrainConfig {
    ...
    #[config(default = 40.0)]
    #[arg(long, help_heading = "Training options", default_value = "40.0")]
...
impl SplatTrainer {
    ...
    async fn refine_splats(
        ...
        let avg_refine_grad =
            refiner.refine_weight_norm / refiner.visible_counts.clamp_min(1).float();
        ...
        if refine_step % self.config.reset_alpha_every_refine == 0 {
            ...
            map_opt::<_, 1>(splats.raw_opacity.id, &mut record, &|s| {
                Tensor::zeros_like(&s).require_grad()
            });
        } else {
...
async fn prune_points(
    ...
    mut refiner: RefineRecord<Wgpu>,
    prune: Tensor<Wgpu, 1, Bool>,
) -> (Splats<TrainBack>, RefineRecord<Wgpu>, u32) {
    assert_eq!(
        prune.dims()[0] as u32,
        splats.num_splats(),
