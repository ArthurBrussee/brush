...
const TOTAL_GRADS = BATCH_SIZE * 9;
var<workgroup> gather_grads: array<f32, TOTAL_GRADS>;
var<workgroup> refine_grads: array<f32, BATCH_SIZE>;
    ...
    for (var b = 0; b < num_batches; b++) {
        ...
        for (var tb = 0; tb < remaining; tb += microbatch_size) {
            ...
            for(var tt = 0; tt < microbatch_size; tt++) {
                ...
                if inside && isect_id < final_isect {
                    ...
                    if (sigma >= 0.0 && alpha >= 1.0 / 255.0) {
                        ...
                        let v_xy_pix = v_xy * vec2f(uniforms.img_size);
                        v_refine = length(v_xy_pix);
                    }
                }
                ...
                if subgroupAny(splat_active) {
                    ...
                    if subgroup_invocation_id == 0 {
                        ...
                        gather_grads[grad_idx * 9 + 0] = v_xy_sum.x;
                        gather_grads[grad_idx * 9 + 1] = v_xy_sum.y;
                        gather_grads[grad_idx * 9 + 2] = v_conic_sum.x;
                        gather_grads[grad_idx * 9 + 3] = v_conic_sum.y;
                        gather_grads[grad_idx * 9 + 4] = v_conic_sum.z;
                        gather_grads[grad_idx * 9 + 5] = v_colors_sum.x;
                        gather_grads[grad_idx * 9 + 6] = v_colors_sum.y;
                        gather_grads[grad_idx * 9 + 7] = v_colors_sum.z;
                        gather_grads[grad_idx * 9 + 8] = v_colors_sum.w;

                        refine_grads[grad_idx] = v_refine_sum;
            ...
            if local_idx < u32(grad_count) {
                let id = gather_grad_id[local_idx];
                write_grads_atomic(gather_grads[local_idx * 9 + 0], id * 9 + 0);
                write_grads_atomic(gather_grads[local_idx * 9 + 1], id * 9 + 1);
                write_grads_atomic(gather_grads[local_idx * 9 + 2], id * 9 + 2);
                write_grads_atomic(gather_grads[local_idx * 9 + 3], id * 9 + 3);
                write_grads_atomic(gather_grads[local_idx * 9 + 4], id * 9 + 4);
                write_grads_atomic(gather_grads[local_idx * 9 + 5], id * 9 + 5);
                write_grads_atomic(gather_grads[local_idx * 9 + 6], id * 9 + 6);
                write_grads_atomic(gather_grads[local_idx * 9 + 7], id * 9 + 7);
                write_grads_atomic(gather_grads[local_idx * 9 + 8], id * 9 + 8);

                let refine_grad = refine_grads[local_idx];

                #ifdef HARD_FLOAT
                    atomicAdd(&v_refine_grad[id], refine_grad);
                #else
                    var old_value = atomicLoad(&v_refine_grad[id]);
                    loop {
                        let cas = atomicCompareExchangeWeak(&v_refine_grad[id], old_value, add_bitcast(old_value, refine_grad));
                        if cas.exchanged { break; } else { old_value = cas.old_value; }
                    }
                #endif
            }
