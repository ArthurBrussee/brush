...
use burn_jit::cubecl::wgpu::{WgpuCompiler, WgpuRuntime};
use burn_jit::cubecl::CubeDim;
use burn_jit::{cubecl, BoolElement, FloatElement, IntElement, JitRuntime};
use tracing::trace_span;

use crate::stats_kernel::stats_gather_kernel;

type Inner<C, F, I, BT> = JitBackend<WgpuRuntime<C>, F, I, BT>;
type Fused<C, F, I, BT> = Fusion<Inner<C, F, I, BT>>;
type Diff<C, F, I, BT> = Autodiff<Fused<C, F, I, BT>>;
...
impl<C: WgpuCompiler, F: FloatElement, I: IntElement, BT: BoolElement>
    RefineRecord<Fused<C, F, I, BT>>
where
    WgpuRuntime<C>: JitRuntime,
{
    pub(crate) fn gather_stats(
        &self,
        xys_grad: Tensor<Fused<C, F, I, BT>, 2>,
        aux: RenderAux<Diff<C, F, I, BT>>,
    ) {
        ...
        let compact_gid = client
            .resolve_tensor_int::<Inner<C, F, I, BT>>(aux.global_from_compact_gid.into_primitive());
        let num_visible =
            client.resolve_tensor_int::<Inner<C, F, I, BT>>(aux.num_visible.into_primitive());
        let radii = client.resolve_tensor_float::<Inner<C, F, I, BT>>(
            aux.radii.inner().into_primitive().tensor(),
        );
        let xys_grad =
            client.resolve_tensor_float::<Inner<C, F, I, BT>>(xys_grad.into_primitive().tensor());

        let inner_client = &compact_gid.client;

        let grad_2d_accum = client.resolve_tensor_float::<Inner<C, F, I, BT>>(
            self.grad_2d_accum.clone().into_primitive().tensor(),
        );
        let grad_counts = client
            .resolve_tensor_int::<Inner<C, F, I, BT>>(self.xy_grad_counts.clone().into_primitive());
        let max_radii = client.resolve_tensor_float::<Inner<C, F, I, BT>>(
    ...
    pub(crate) fn average_grad_2d(&self) -> Tensor<Fused<C, F, I, BT>, 1> {
        self.grad_2d_accum.clone() / self.xy_grad_counts.clone().clamp_min(1).float()
    }

    pub(crate) fn max_radii(&self) -> Tensor<Fused<C, F, I, BT>, 1> {
