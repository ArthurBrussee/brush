...
impl<B: Backend + SplatBackwardOps<B>> Backward<B, NUM_ARGS> for RenderBackwards {
    ...
    fn backward(
        ...
        if let Some(node) = refine_weight {
            grads.register::<B>(node.id, v_tens.refine_weight);
        }
    ...
    fn render_splats_bwd(
            ...
            Operation<FusionJitRuntime<WgpuRuntime, BT>> for CustomOp<F, I, BT>
        {
            fn execute(self: Box<Self>, h: &mut HandleContainer<JitFusionHandle<WgpuRuntime>>) {
                ...
                h.register_float_tensor::<BBase<F, I, BT>>(&v_xy.id, grads.refine_weight);
            }
        }
        ...
        let grads = SplatGrads::<Self> {
            ...
            refine_weight: client.tensor_uninitialized(vec![num_points], DType::F32),
        };
