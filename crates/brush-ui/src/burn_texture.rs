...
impl BurnTexture {
    ...
    pub fn update_texture(&mut self, img: Tensor<BInner, 3>) -> TextureId {
        ...
        let img = fusion_client.resolve_tensor_float::<BJit>(img_prim);
        let img: Tensor<BJit, 3> = Tensor::from_primitive(TensorPrimitive::Float(img));
        ...
        let img = if width % 64 != 0 {
            let padded: Tensor<BJit, 3> = Tensor::zeros(&padded_shape, &img.device());
