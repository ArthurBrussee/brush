...
//! even coverage at a controllable density. This is a far richer init than the
//! random in-frustum fallback used when there's no `SfM` point cloud.
...
pub async fn lidar_init_splats(
    ...
    // sampled color, tracking the cloud extent for auto voxel sizing.
    ...
    for view in views {
        ...
        // Intrinsics at the depth map's native resolution (LiDAR is aligned to
        // the RGB camera's fov). Pinhole back-projection ignores lens
        ...
        for v in 0..h {
            for u in 0..w {
                ...
                // Color from the matching image pixel (depth and RGB share the
                // camera fov, different resolutions).
